function renderWheel() {
  const page = document.getElementById('page-wheel');
  page.innerHTML = `
    <div class="tool-card">
      <h2>🎡 轉盤</h2>
      <p>輸入選項（一行一個），按轉盤隨機選一個</p>
      <textarea id="wheelList" rows="5">第1組
第2組
第3組
第4組
第5組
第6組</textarea>
      <div class="wheel-container">
        ...
        <div class="wheel-center" id="wheelSpinBtn">轉！</div>
      </div>
      <div class="wheel-result" id="wheelResult">點中間按鈕開始轉</div>
    </div>
  `;
  ...
  function drawWheel(rotation) {
    ...
    items.forEach((item, i) => {
      ...
      const display = item.length > 6 ? item.slice(0, 6) + '..' : item;
      ctx.fillText(display, r - 20, 6);
      ctx.restore();
    });
  ...
  function getWinner(rotation) {
    ...
    const slice = (2 * Math.PI) / items.length;
    const adjusted = (2 * Math.PI - (rotation % (2 * Math.PI))) % (2 * Math.PI);
  ...
  document.getElementById('wheelSpinBtn').addEventListener('click', () => {
    ...
    const targetSpin = spins * 2 * Math.PI + Math.random() * 2 * Math.PI;
